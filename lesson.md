...
4. **Cosine similarity is a ranking signal, not a percentage.** Modern sentence-transformer cosines mostly live in `[0.2, 0.7]`. A 0.25 between *frock* and *dress* is **not** "25% similar" — it is a meaningful lift above the ~0.0 baseline for unrelated words. Read cosines as a ranking, not a score out of 1.
...
> **Sample answer:** No. Sentence-transformer cosines for this model mostly live in the range [0.2, 0.7]. A 0.25 is **not** "25% similar"; it is a ranking signal that sits well above the ~0.0 baseline for unrelated words and is meaningfully high for a single-word synonym pair. The right way to use cosines is as a way to rank candidates, not as a percentage to compare against 100%.
